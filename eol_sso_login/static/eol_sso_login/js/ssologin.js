function limitTextarea(textarea, maxLines) {      
  ...
  function enrollrun(e){      
    ...
    var datos = document.getElementById("student-run");
  ...
  validate_success = function(data) {
    ...
    if ("saved" in data && data.saved == "saved"){          
      if (data.run_saved['run_saved_pending'] != ""){
        aux_success = aux_success + "<b>No se ha encontrado ninguna cuenta institucional de los siguientes ruts: </b></br>" + data.run_saved['run_saved_pending'] + "</br></br>";
        aux_success = aux_success + "Al momento de registrarse, automaticamente se inscribirán en el curso.</br></br>";
      }
      if (data.run_saved['run_saved_not_found'] != ""){
        aux_success = aux_success + "No se ha encontrado ninguna cuenta de los siguientes ruts: " + data.run_saved['run_saved_not_found'] + "</br>";
      }
